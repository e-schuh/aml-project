...
def parse_args():
    ...
    parser.add_argument(
        "--intrasentence-gold-file-path",
        type=str,
        default=None,
        help="Path to intrasentence gold file.",
        )
    parser.add_argument(
        "--intersentence-gold-file-path",
        type=str,
        default=None,
        help="Path to intersentence gold file.",
        )
    parser.add_argument(
        "--inference-output-file-path",
        type=str,
        default=None,
        help="Path to inference output file of the combined results.",
        )
    parser.add_argument(
        "--skip-intrasentence",
        action="store_true",
        help="Do not perform intrasentence evaluation.",
        )
    parser.add_argument(
        "--skip-intersentence",
        default=True,
        action="store_true",
        help="Do not perform intersentence evaluation.",
        )
    parser.add_argument(
        "--output-file",
        type=str,
...
class ScoreStorage(object):
    def __init__(self):
        ...
        Args:
            ...
            - inference_output_file_path : path, relative or absolute, to the inference output file
        Returns:
...
if __name__ == "__main__":
    ...
    if args.output_file is None:
        args.output_file = os.path.join(args.top_level_dir, "evaluation_output", "eval_metrics.json")
    
    os.makedirs(os.path.dirname(args.output_file), exist_ok=True)

    with open(args.inference_output_file_path) as f:
        predictions = json.load(f)

    score_evaluator = ScoreStorage()
    results = defaultdict(lambda: {})
    if not args.skip_intrasentence:
        df_intrasentence = pd.read_pickle(args.intrasentence_gold_file_path)
        target_col = "target" if os.path.basename(args.intrasentence_gold_file_path)[17:19] == "en" else "target_original"
    ...
    if not args.skip_intersentence:
        df_intersentence = pd.read_pickle(args.intersentence_gold_file_path)
        target_col = "target" if os.path.basename(args.intersentence_gold_file_path)[17:19] == "en" else "target_original"
    ...
    if args.output_file is not None:
        ...
        model_names = os.path.basename(args.inference_output_file_path)[:-5]
